 ...
 * for documents, snippets and search results.</p>
 *
 * <p>
 * This class is set up to work with an embedded Jetty create, but it should be
 * fairly easy to wrap into a Servlet for use with something else (Tomcat,
 * Glassfish, etc.)</p>
 *
 * <p>
 * URLs supported:</p>
...
//                    item.identifier,
    ...
    protected SearchResult performSearch(HttpServletRequest request, boolean snippets) throws Exception {
        ...
        String qtype = search.retrieval.getGlobalParameters().get("queryType", "complex");
        // allow override at the request level if specified
        if (request.getParameterValues("queryType") != null) {
            qtype = request.getParameterValues("queryType")[0];
        }
